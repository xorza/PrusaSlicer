...
double distance_to_squared(const L &line, const Vec<N, T> &point, Vec<N, T> *nearest_point)
{
    ...
    if (t < 0.0) {
        ...
        *nearest_point = line.b;
        return (point - line.b).template cast<double>().squaredNorm();
    }

    *nearest_point = (line.a.template cast<double>() + t * v).template cast<T>();
...
{
    Vec<N, T> nearest_point;
    return distance_to_squared<L, T, N>(line, point, &nearest_point);
}
...
public:
    ...
    double distance_to_squared(const Point &point, Point *nearest_point) const { return line_alg::distance_to_squared(*this, point, nearest_point); }
