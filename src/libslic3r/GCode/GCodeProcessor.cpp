...
void GCodeProcessor::process_T(const std::string_view command)
{
    if (command.length() > 1) {
        ...
        if (! parse_number(command.substr(1), eid) || eid < 0 || eid > 255) {
            // T-1 is a valid gcode line for RepRap Firmware (used to deselects all tools) see https://github.com/prusa3d/PrusaSlicer/issues/5677
            if (m_flavor != gcfRepRapFirmware || eid != -1)
