...
void RemovableDriveManager::eject_drive()
{
	...
	if (it_drive_data != m_current_drives.end()) {
		...
		BOOL e3 = DeviceIoControl(handle, IOCTL_STORAGE_MEDIA_REMOVAL, &Prevent, sizeof(PREVENT_MEDIA_REMOVAL), nullptr, 0, &deviceControlRetVal, nullptr);
		BOOST_LOG_TRIVIAL(debug) << "IOCTL_STORAGE_MEDIA_REMOVAL " << e3 << " ; " << deviceControlRetVal << " ; " << GetLastError();
